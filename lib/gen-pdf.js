...
function renderClassic(doc, data, PW, ML) {
  ...
  leftY += 14;
  const receiverLines = [data.receiverName, data.receiverAddress].filter(Boolean);
  ...
  for (let i = 0; i < rowsToRender; i++) {
    ...
    const persons = item && item.persons ? +item.persons : 0;
    const days = item && item.days ? +item.days : 1;
