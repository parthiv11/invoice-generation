...
function renderClassic(doc, data, PW, ML) {
  ...
  const past = +(data.pastBalanceDue ?? 0) || 0;
  const totals = [
    ["", "", "TOTAL AMOUNT BEFORE TAX", fmtAmt(t.sub)],
    ["", "", `CGST ${cp}%`, fmtAmt(t.cgst)],
    ["", "", `SGST ${gp}%`, fmtAmt(t.sgst)],
    ["", "", "PAST BALANCE DUE", fmtAmt(past)],
...
function genPDF(data) {
  return new Promise((resolve, reject) => {
    ...
    totRow("Total Amount Before Tax",       fmtINR(t.sub), true);
    totRow(`CGST ${cp}%`,                  fmtINR(t.cgst));
    totRow(`SGST ${gp}%`,                  fmtINR(t.sgst), false);
    totRow("PAST BALANCE DUE",             fmtINR(past), true);
