...
async function genXLSX(data) {
  ...
  ws.mergeCells(`A${R}:F${R}`);
  cell(ws, `A${R}`, "ORIGINAL FOR RECIPIENT", { bold:true, size:8, color:ORANGE, align:"center", bg:WHITE });
  ...
  for (let row = 0; row < 4; row++) {
    ...
    const leftFill = row === 3 ? LGREY : WHITE;
    const rightFill = row === 3 ? SKYBLUE : WHITE;
  ...
  cell(ws, `A${sigStart + 3}`, "Authorised Signatory", { bold:true, size:9, color:"FF1F3864", bg:LGREY, align:"center" });
  cell(ws, `D${sigStart + 3}`, `by ${data.supplierName}`, { bold:true, size:9, color:"FF1F3864", bg:SKYBLUE, align:"center" });
  R += 4;
